...
import { Mail, Lock, User, ArrowRight, Loader2, Trophy, Eye, EyeOff } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to register. Please check your Firebase config.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden px-4 py-12">
      {/* MSU Logo Background - Maximized Visibility */}
      <div className="absolute inset-0 z-0 pointer-events-none flex items-center justify-center overflow-hidden">
        <img 
          src="/msu-logo.png" 
          alt="MSU Background" 
          className="w-full h-full object-contain opacity-[0.35] blur-[4px] scale-100" 
        />
      </div>
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        <div className="absolute top-[-10%] left-[-5%] w-[40%] h-[40%] bg-accent/5 rounded-full blur-[100px]" />
        ...
        <div className="bg-white/75 backdrop-blur-2xl rounded-[2.5rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/40 relative overflow-hidden">
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Official Full Name</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">University Email Address</label>
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="student@medhavi.university" 
                  className="w-full h-14 bg-slate-50 border border-slate-200 rounded-2xl px-12 text-sm font-bold tracking-tight focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all text-slate-900 placeholder:text-slate-400"
                />
              </div>
            </div>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-[10px] uppercase tracking-[0.3em] font-black text-slate-300">
              <span className="bg-white px-6">Fast-Track Entry</span>
            </div>
          </div>
