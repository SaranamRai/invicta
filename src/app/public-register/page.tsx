...
export default function PublicRegisterPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!tournamentId || !sportId || !text(department) || !captain.name || !captain.registrationNo || !captain.email || captain.phone.length !== 10) { setStatus("error"); setMessage("Please complete all required captain and team details."); return; }
    if (cleanMembers.length < requiredMembers || cleanMembers.length > maxMembers || cleanMembers.some((member) => !member.fullName || !member.registrationNumber || !member.email)) { setStatus("error"); setMessage(`Please enter ${requiredMembers} to ${maxMembers} team members with name, registration number, and email.`); return; }
    const registrationNumbers = [captain.registrationNo, ...cleanMembers.map((member) => member.registrationNumber)]; const emails = [captain.email, ...cleanMembers.map((member) => member.email)];
    if (new Set(registrationNumbers).size !== registrationNumbers.length || new Set(emails).size !== emails.length) { setStatus("error"); setMessage("Each player must have a unique registration number and email."); return; }
    setStatus("submitting"); setMessage("");
    try {
      const departmentName = text(department).toUpperCase(); const result = await registerPublicTeam({ name: departmentName, teamName: departmentName, department: departmentName, tournamentId, tournamentName: selectedTournament?.name || "", sportId, sportName: selectedSport?.sportName || selectedSport?.name || "", category, members: cleanMembers, captainName: captain.name, captainRegNo: captain.registrationNo, captainEmail: captain.email, captainPhone: captain.phone, contactNumber: captain.phone, email: captain.email, phone: captain.phone, status: "pending" });
      setStatus("success"); setMessage(`${result.name || "Your team"} is registered and awaiting super coordinator approval.`); setCaptainName(""); setCaptainRegNo(""); setEmail(""); setPhone(""); setDepartment(""); setMembers([]);
