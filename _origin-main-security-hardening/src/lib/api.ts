...
export async function apiFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  return apiFetchWithRefresh<T>(path, options, true);
}

async function apiFetchWithRefresh<T>(path: string, options: RequestInit = {}, allowRefresh: boolean): Promise<T> {
  ...
  if (!response.ok) {
    if (response.status === 401 && allowRefresh && !path.startsWith("/auth/login") && !path.startsWith("/auth/refresh")) {
      ...
      if (refreshed) {
        return apiFetchWithRefresh<T>(path, options, false);
      }
    }
...
export interface SecurityMonitoringSummary {
  ...
  apiSamples: number;
  slowApis: SecurityLogEntry[];
  recentErrors: SecurityLogEntry[];
  recentAudits: SecurityLogEntry[];
}

export function getSecurityMonitoring() {
  return apiFetch<SecurityMonitoringSummary>("/admin/security/monitoring");
}

export function getAuditLogs() {
  return apiFetch<SecurityLogEntry[]>("/admin/security/audit-logs");
}

export function getErrorLogs() {
  return apiFetch<SecurityLogEntry[]>("/admin/security/error-logs");
}

export async function downloadSecurityLog(kind: "audit" | "error") {
  const endpoint = kind === "audit" ? "/admin/security/audit-logs.csv" : "/admin/security/error-logs.csv";
  const { blob, contentDisposition } = await apiDownload(endpoint);
  const fallback = kind === "audit" ? "invicta-audit-logs.csv" : "invicta-error-logs.csv";
