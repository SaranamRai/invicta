...
app.use(
  cors({
    origin: function (origin, callback) {
      ...
      const isLocalOrigin = /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(cleanOrigin);

      if (process.env.NODE_ENV === "production" && cleanOrigin.startsWith("http://") && !isLocalOrigin) {
        return callback(new Error("HTTPS origin required"));
      }
